...
-- Idempotent: replaces this sermon and its children on re-run.
-- Run after 01_schema.sql.
--
-- >> READ THE `notes` COLUMN BELOW BEFORE USING THIS RECORD.
-- >> Source is the preacher's SLIDE DECK, not a transcript, and two
-- >> personal family testimonies have been redacted for privacy.
-- ============================================================

delete from sermons where service_date = '2026-08-02'
  and title = 'Family Altar';

with s as (
  insert into sermons (service_date, title, preacher, is_guest, occasion, series, anchor_scripture, thesis, summary, full_text, youtube_url, notes)
  values (
    '2026-08-02',
    'Family Altar',
    'Dr Titi Sodipo',
    ...
    'PREACHER''S OWN SLIDE NOTES (not a transcript).
...
ins_scr as (
  ...
  values
    ((select id from s), 'Genesis 18:18-19', 'Genesis', 18, 18, 19, 'anchor'),
    ((select id from s), 'Galatians 4:19', 'Galatians', 4, 19, 19, 'anchor'),
    ((select id from s), 'Matthew 18:20', 'Matthew', 18, 20, 20, 'supporting'),
    ((select id from s), 'Job 1:8-10', 'Job', 1, 8, 10, 'supporting'),
    ((select id from s), 'Psalm 91:9-11', 'Psalm', 91, 9, 11, 'supporting'),
    ((select id from s), '2 Timothy 1:5', '2 Timothy', 1, 5, 5, 'supporting'),
    ((select id from s), 'Ecclesiastes 10:10', 'Ecclesiastes', 10, 10, 10, 'supporting'),
    ((select id from s), 'Romans 10:9', 'Romans', 10, 9, 9, 'supporting'),
    ((select id from s), 'Romans 10:14-15', 'Romans', 10, 14, 15, 'supporting'),
    ((select id from s), 'Romans 12:1', 'Romans', 12, 1, 1, 'supporting'),
    ((select id from s), 'Proverbs 10:6-8', 'Proverbs', 10, 6, 8, 'supporting'),
    ((select id from s), '2 Timothy 3:16-17', '2 Timothy', 3, 16, 17, 'supporting'),
    ((select id from s), 'Acts 17:11', 'Acts', 17, 11, 11, 'supporting'),
    ((select id from s), 'John 15:10-11', 'John', 15, 10, 11, 'supporting'),
    ((select id from s), 'Hebrews 6:1-2', 'Hebrews', 6, 1, 2, 'supporting'),
    ((select id from s), 'Hebrews 5:12-14', 'Hebrews', 5, 12, 14, 'supporting'),
    ((select id from s), 'Galatians 3:1-3', 'Galatians', 3, 1, 3, 'supporting'),
    ((select id from s), 'Galatians 5:16-21', 'Galatians', 5, 16, 21, 'supporting'),
    ((select id from s), 'Philippians 4:6', 'Philippians', 4, 6, 6, 'supporting'),
    ((select id from s), 'Proverbs 22:6', 'Proverbs', 22, 6, 6, 'supporting')
  returning 1
)
,
ins_clips as (
  ...
  values
    ((select id from s), 1, 'Don''t depend on Sunday school to train your children', 'parenting / discipleship', null, '{reel,short,tiktok}'::text[], 'to_caption', false, null),
    ((select id from s), 2, 'Training is not the same as telling: you do it and they watch, you do it together, they do it and you watch, they do it alone', 'how to train children', null, '{reel,carousel}'::text[], 'to_caption', false, null),
    ((select id from s), 3, 'When you read about baptism, you can baptise in the bath', 'faith at home / obedience', null, '{reel,short}'::text[], 'to_caption', false, null),
    ((select id from s), 4, 'God chose Abraham because he would train his household - and THEN the promise came', 'Genesis 18:19', null, '{reel,short}'::text[], 'to_caption', false, null),
    ((select id from s), 5, 'Keep interceding until Christ is fully formed in them', 'praying for your children', null, '{reel,short,tiktok}'::text[], 'to_caption', false, null),
    ((select id from s), 6, 'Personal family testimonies from the preacher''s own household', 'testimony', null, '{reel}'::text[], 'to_caption', true, 'Preacher''s own family stories, one involving a family member''s health. Detail deliberately not recorded here. Written consent from the preacher required before clipping or publishing, and external comms approval per Governance section 11.')
  returning 1
)
,
ins_tags as (
...
select (select id from s), t.id from sermon_tags t
where t.name in ('family altar','discipleship','parenting','prayer','bible study','altars','generational faith')
on conflict do nothing;

-- VERIFY
select service_date, title, preacher, is_guest, series, anchor_scripture,
